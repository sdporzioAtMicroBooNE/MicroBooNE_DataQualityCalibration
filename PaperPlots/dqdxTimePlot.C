...
void dqdxTimePlot()
{
  // OPEN FILE
  ifstream inFile;
  inFile.open("/uboone/app/users/sporzio/CalibrationXYZ/Output/T/dqdx.txt");
  ...
  tg->SetTitle("");
  tg->GetYaxis()->SetTitle("Global Median dQ/dx (ADC/cm)");
  tg->GetXaxis()->SetTitle("Day");
  tg->GetXaxis()->SetRangeUser(0,140);
  tg->GetYaxis()->SetRangeUser(175,220);
  gPad->SetTicks();
  // Draw histogram
  tg->Draw("AP");
  // Draw line
  TLine *line = new TLine(0,210,140,210);
  line->SetLineColor(kRed);
  line->SetLineStyle(7);
  line->SetLineWidth(3);
  line->Draw("SAME");
  // Save canvas to file
  c1->Print("Plots/run3_dqdxMedian_time.pdf");
